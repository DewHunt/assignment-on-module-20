...
const AddEditForm = ({ blogInfo, categories, btnName, action }) => {
  ...
  const formSubmit = async (e) => {
    ...
    if (IsEmpty(data.categoryId)) {
      ...
      ErrorToast("Blog Long Description Required");
    } else if (IsEmpty(data.img1)) {
      ErrorToast("Blog First Image Required");
    } else {
      setSubmit(true);
      const options = {
        method: "POST",
      ...
      const result = await fetch("/api/blog/upsert", options);
      const resultJson = await result.json();
      setSubmit(false);
      if (resultJson["status"] === "success") {
        let msg = "Blog Saved Sucessfull";
        if (action === "update") {
        ...
        <div className="card-header">
          <div className="row">
            ...
            <div className="col-xxl-6 col-xl-6 col-lg-6 col-md-6 col-sm-6 col-12 text-end">
              <Link className="btn btn-outline-danger" href="/dashboard/blog">
                All Blogs
              </Link>
